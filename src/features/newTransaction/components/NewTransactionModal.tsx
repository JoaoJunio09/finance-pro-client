...
function TransactionModal({ isOpen, onClose }: TransactionModalProps) {
  ...
  // Estados específicos de Recorrência
  const [freq, setFreq] = useState<FrequencyType>('MONTHLY');

  const inputRef = useRef<HTMLInputElement>(null);

  // Focus automático no valor ao abrir
  useEffect(() => {
    if (isOpen) {
      ...
      setTimeout(() => {
        setType('out'); setAmountStr(''); setDescription(''); setCategoryId(''); setWalletId(''); setDate(getTodayDate());
      }, 300);
    }
  }, [isOpen]);
  ...
  const { form, handleOnChange, setFrequency, setType, type, categories, wallets } = useNewTransaction();
      ...
      <div className="relative w-full sm:max-w-5xl bg-[#09090B] sm:border border-white/[0.06] rounded-t-[32px] sm:rounded-[28px] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.8)] flex flex-col sm:flex-row animate-bottom-sheet sm:animate-modal h-[95vh] sm:h-auto sm:max-h-[90vh] overflow-hidden flex-shrink-0 z-10 min-h-0">
        ...
        <div className="w-full sm:w-[340px] bg-[#111113] border-t sm:border-t-0 sm:border-l border-white/[0.04] flex flex-col shrink-0 z-20 min-h-0">
            ...
            <div className="bg-[#09090B] border border-white/[0.04] rounded-2xl p-5 shadow-inner">
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <span className="text-xl font-bold text-white tracking-tight">{amountStr || 'R$ 0,00'}</span>
                </div>
                
                <div>
                  <span className="text-xs text-zinc-500 block mb-1">Descrição</span>
                  <span className={`text-sm font-medium ${description ? 'text-zinc-200' : 'text-zinc-600 italic'}`}>
                  ...
                  <div className="flex-1">
                    ...
                    {selectedCategory ? (
                      <div className="flex items-center gap-1.5 text-xs font-medium text-zinc-300">
                        <div className={`w-4 h-4 rounded ${selectedCategory.bg} flex items-center justify-center ${selectedCategory.color}`}>
                          <selectedCategory.icon className="w-2.5 h-2.5" />
                        </div>
                ...
                {!isRecurring && (
                  <div className="pt-2">
                    ...
                    <span className="text-xs font-medium text-zinc-300">{date === getTodayDate() ? 'Hoje, ' : ''}{formatDateBR(date)}</span>
                  </div>
                )}
                ...
                {isRecurring && (
                  <div className="pt-2">
                    ...
                    <span className="text-xs font-medium text-zinc-300 capitalize">{freq}</span>
                  </div>
                )}
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 p-6 sm:p-8 pt-6 sm:pt-0 shrink-0 bg-[#09090B] sm:bg-[#111113] border-t border-white/[0.04] sm:border-t-0">
            <button 
              className={`w-full py-4 rounded-xl text-sm font-bold tracking-wide transition-all duration-300 shadow-xl active:scale-[0.98] ${
            ...
            <button 
              onClick={onClose}
              className="w-full py-4 rounded-xl bg-transparent hover:bg-white/[0.04] text-zinc-400 hover:text-white text-sm font-medium transition-colors"
            >
              Cancelar
            </button>
          </div> 
