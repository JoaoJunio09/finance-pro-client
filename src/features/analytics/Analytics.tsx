    ...
    <div className="w-full bg-[var(--bg-surface)]">
      <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 pt-6 sm:pt-8 pb-0 flex flex-col gap-6 sm:gap-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* Sub-header Contextual - Analytics */}
          <div className="flex items-center gap-4 self-start sm:self-center w-full sm:w-auto mt-3 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-t-0 border-[var(--border-light)]">
            <div className="hidden sm:flex flex-col items-end">
              <span className="text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider">Período Analisado</span>
              <span className="text-sm font-semibold text-[var(--text-main)] flex items-center gap-1.5 mt-0.5">
                <Calendar size={14} className="text-[var(--accent)]" /> Julho, 2026
              </span>
            </div>
            <div className="h-8 w-px bg-[var(--border-light)] hidden sm:block mx-1"></div>
            <button 
              onClick={setSubpageToReports}
              className="flex items-center justify-center gap-2 text-xs font-semibold text-[var(--text-main)] bg-[var(--bg-surface)] px-4 py-2.5 rounded-xl border border-[var(--border-color)] shadow-sm hover:border-[var(--border-hover)] hover:bg-[var(--bg-elevated)] transition-all cursor-pointer w-full sm:w-auto group"
            >
              <Download size={15} className="text-[var(--text-muted)] group-hover:text-[var(--accent)] transition-colors" />
...
const DashboardPage = ({ data, setMainView, setSubpage }: any) => {
  const { summary, categories, recentTransactions } = data;
  ...
  // Componente interno para os cards menores da visão geral
  const DashboardMetricCard = ({ title, amount, icon: Icon, colorClass, isAccent = false }: any) => (
    <div className={`interactive-card rounded-3xl p-5 sm:p-6 flex flex-col justify-between gap-4 shadow-sm border ${isAccent ? 'bg-[var(--accent-muted)] border-[var(--accent)]/30' : 'bg-[var(--bg-surface)] border-[var(--border-color)]'}`}>
      <div className="flex justify-between items-start">
        <span className={`text-xs sm:text-sm font-medium ${isAccent ? 'text-[var(--accent)]' : 'text-[var(--text-muted)]'}`}>{title}</span>
        <div className={`w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center bg-[var(--bg-elevated)] border border-[var(--border-light)] ${colorClass} bg-opacity-10`}>
          <Icon size={18} />
        </div>
      </div>
      <span className={`text-xl sm:text-2xl font-bold tracking-tight tabular-nums truncate ${isAccent ? 'text-[var(--accent)]' : 'text-[var(--text-main)]'}`}>
        {showBalance ? formatCurrency(amount) : 'R$ •••••'}
      </span>
    </div>
  );
  ...
  return (
    <div className="flex flex-col gap-6 sm:gap-8 animate-fade-in-up">
      ...
      <section className="relative overflow-hidden rounded-[2rem] bg-gradient-to-r from-[var(--accent-muted)] to-transparent border border-[var(--border-light)] p-6 sm:p-8 flex flex-col sm:flex-row sm:items-center justify-between gap-6 shadow-sm">
        {/* Background Decorativo Suave */}
        <div className="absolute top-0 right-0 -mt-10 -mr-10 w-40 h-40 bg-[var(--accent)]/10 rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-0 left-20 w-32 h-32 bg-[var(--accent)]/10 rounded-full blur-3xl pointer-events-none"></div>

        <div className="flex flex-col gap-2 relative z-10">
          <div className="flex items-center gap-2">
            <span className="text-[var(--accent)] font-semibold text-[10px] uppercase tracking-widest bg-[var(--bg-surface)]/60 backdrop-blur-md px-2.5 py-1 rounded-full border border-[var(--border-light)]">
              Seu resumo de hoje
            </span>
          </div>
          <h1 className="text-2xl sm:text-3xl font-bold text-[var(--text-main)] tracking-tight mt-1">
            Bom dia, João 👋
          </h1>
          <p className="text-sm text-[var(--text-muted)] font-medium max-w-md leading-relaxed">
            Aqui está o panorama da sua vida financeira para você acompanhar tudo de forma rápida.
          </p>
        </div>
        
        {/* Sub-header Contextual - Dashboard */}
        <div className="relative z-10 flex items-center justify-between sm:justify-end gap-4 sm:gap-6 self-stretch sm:self-center shrink-0 mt-2 sm:mt-0 pt-4 sm:pt-0 border-t sm:border-t-0 border-[var(--border-light)]">
          <div className="flex flex-col sm:items-end">
            <span className="text-[10px] font-bold text-[var(--accent)] uppercase tracking-wider mb-1 hidden sm:block">Ritmo de Gastos</span>
            <span className="text-sm font-semibold text-[var(--text-main)] flex items-center gap-1.5">
              <Activity size={15} className="text-[var(--accent)]" /> 17 dias restantes no mês
            </span>
            <span className="text-[11px] font-medium text-[var(--text-muted)] mt-0.5">Limite diário seguro: <span className="font-semibold text-[var(--text-main)]">{showBalance ? 'R$ 225,00' : '••••'}</span></span>
          </div>
          <button 
            onClick={() => setShowBalance(!showBalance)}
            className="flex items-center justify-center w-11 h-11 sm:w-12 sm:h-12 rounded-2xl bg-[var(--bg-surface)] border border-[var(--border-color)] text-[var(--text-muted)] hover:text-[var(--accent)] hover:border-[var(--accent)]/50 transition-all shadow-sm group shrink-0"
            title={showBalance ? "Ocultar valores" : "Mostrar valores"}
          >
            {showBalance ? <EyeOff size={20} className="group-hover:scale-110 transition-transform" /> : <Eye size={20} className="group-hover:scale-110 transition-transform" />}
          </button>
        </div>
      </section>
      ...
      <section className="flex flex-col gap-4 sm:gap-6">
        ...
        {/* Saldo Atual com forte Hierarquia Visual (Roxo Escuro) - Estilo referenciado no Analytics */}
        <div className="w-full bg-gradient-to-br from-[#4C1D95] to-[#312E81] rounded-3xl p-6 sm:p-10 flex flex-col justify-center shadow-md relative overflow-hidden text-white border border-[#5B21B6]/50 interactive-card">
          <div className="absolute inset-0 bg-[url('data:image/svg+xml;base64,PHN2ZyB3aWR0aD0iMjAiIGhlaWdodD0iMjAiIHhtbG5zPSJodHRwOi8vd3d3LnczLm9yZy8yMDAwL3N2ZyI+PGNpcmNsZSBjeD0iMiIgY3k9IjIiIHI9IjIiIGZpbGw9IiNmZmZmZmYiIGZpbGwtb3BhY2l0eT0iMC4wNSIvPjwvc3ZnPg==')] opacity-20"></div>
          
          <div className="relative z-10 flex flex-col gap-2">
            <span className="text-sm font-medium text-white/80 flex items-center gap-2 bg-white/10 backdrop-blur-sm px-3 py-1.5 rounded-full w-fit border border-white/20">
              <Wallet size={16} className="text-white" />
              Saldo Atual Total
            </span>
            <div className="flex items-baseline gap-3 mt-2">
              <span className="text-5xl sm:text-6xl font-black tracking-tight text-white tabular-nums drop-shadow-sm">
                {showBalance ? formatCurrency(summary.netBalance.value + 12050) /* Usando base fictícia + saldo líquido para ilustração do saldo total */ : 'R$ •••••'}
              </span>
            </div>
          </div>
          ...
          <div className="relative z-10 mt-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div className="flex items-center gap-2 text-xs font-medium text-white/90">
              <ShieldCheck size={16} className="text-[var(--income)]" /> Tudo certo e atualizado com suas contas
            </div>
          </div>
        </div>

        {/* Demais Indicadores - Contexto Auxiliar */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
          <DashboardMetricCard title="Receitas no período" amount={summary.income.value} icon={ArrowUpRight} colorClass="text-[var(--income)]" />
          <DashboardMetricCard title="Despesas no período" amount={summary.expense.value} icon={ArrowDownRight} colorClass="text-[var(--expense)]" />
          <DashboardMetricCard title="Disponível para Gastar" amount={summary.availableToSpend} icon={CheckCircle2} colorClass="text-[var(--accent)]" isAccent={true} />
        </div>
      </section>
      ...
      <section className="flex flex-col gap-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          {wallets.map((wallet) => (
            <div 
              key={wallet.id} 
              className={`relative p-6 rounded-[1.25rem] aspect-[1.58/1] bg-gradient-to-br ${wallet.gradient} shadow-md overflow-hidden flex flex-col justify-between interactive-card ${wallet.textDark ? 'border border-[var(--border-color)] text-[var(--text-main)]' : 'border border-white/10 text-white'}`}
            >
              ...
              <div className="flex justify-between items-start relative z-10">
                <div className="flex items-center gap-2">
                   {wallet.icon === 'nubank' && <span className="font-bold tracking-tight text-lg">nubank</span>}
                   {wallet.icon === 'itau' && <span className="font-bold tracking-tighter text-xl text-[#002B5E] bg-[#EC7000] px-1 rounded-sm">Itaú</span>}
                   {wallet.icon === 'wallet' && <div className="flex items-center gap-1.5"><Wallet size={18} className="text-[var(--text-muted)]"/><span className="font-semibold text-sm text-[var(--text-muted)]">Físico</span></div>}
                </div>
                {/* Imitação visual de um "Chip" bancário */}
                <div className={`w-8 h-6 rounded flex items-center justify-center ${wallet.textDark ? 'bg-[var(--border-color)] border border-[var(--border-hover)]' : 'bg-white/20 border border-white/30'}`}>
                  <div className={`w-5 h-3 rounded-sm border ${wallet.textDark ? 'border-[var(--border-hover)]' : 'border-white/20'}`}></div>
                </div>
              </div>

              <div className="relative z-10">
                <p className={`text-[10px] uppercase tracking-widest font-medium mb-1 ${wallet.textDark ? 'text-[var(--text-muted)]' : 'text-white/70'}`}>{wallet.name}</p>
                <p className="text-2xl sm:text-3xl font-bold tabular-nums tracking-tight">
                  {showBalance ? formatCurrency(wallet.balance) : 'R$ •••••'}
                </p>
              </div>
            </div>
          ))}
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="interactive-card bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-[2rem] p-6 sm:p-8 flex flex-col gap-5 shadow-sm">
          <div className="flex justify-between items-center">
            <h3 className="text-base font-semibold text-[var(--text-main)]">Gasto vs Orçamento</h3>
            <button onClick={() => { setMainView('analytics'); setSubpage('cashflow'); }} className="text-xs font-medium text-[var(--text-muted)] hover:text-[var(--accent)] transition-colors">
              Analisar fluxo
            </button>
          </div>
          ...
          <div className="flex flex-col gap-5 mt-2">
            <div className="flex flex-col gap-2">
              <div className="flex justify-between items-end">
                ...
                <span className="text-base sm:text-lg font-bold text-[var(--income)]">{showBalance ? formatCurrency(summary.income.value) : '••••'}</span>
              </div>
              <ProgressBar percentage={100} colorHex="var(--income)" />
            </div>

            <div className="flex flex-col gap-2 mt-1">
              <div className="flex justify-between items-end">
                <span className="text-sm font-medium text-[var(--text-muted)]">Saídas Consolidadas</span>
                <span className="text-base sm:text-lg font-bold text-[var(--expense)]">{showBalance ? formatCurrency(summary.expense.value) : '••••'}</span>
              </div>
              <ProgressBar percentage={(summary.expense.value / summary.income.value) * 100} colorHex="var(--expense)" />
            </div>
          </div>

          <div className="mt-auto pt-5 border-t border-[var(--border-light)] flex items-start gap-3">
             <div className="w-8 h-8 rounded-full bg-[var(--accent-muted)] flex items-center justify-center text-[var(--accent)] shrink-0">
               <Info size={16} />
             </div>
             <p className="text-xs text-[var(--text-muted)] font-medium leading-relaxed">
               Você comprometeu <span className="font-bold text-[var(--text-main)]">{((summary.expense.value / summary.income.value) * 100).toFixed(0)}%</span> da sua receita neste período. Sua meta é manter abaixo de 70%.
             </p>
          </div>
        </div>
        ...
        <div className="interactive-card bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-[2rem] p-6 sm:p-8 flex flex-col gap-5 shadow-sm">
          <div className="flex justify-between items-center">
            <h3 className="text-base font-semibold text-[var(--text-main)]">Onde você mais gastou</h3>
            <button onClick={() => { setMainView('analytics'); setSubpage('categories'); }} className="text-xs font-medium text-[var(--accent)] hover:underline flex items-center gap-1 transition-all">
              Ver todas <ChevronRight size={14} />
            </button>
          </div>

          <div className="flex flex-col gap-5 mt-1">
            {categories.slice(0, 3).map((cat: ExpenseCategory) => (
              <div key={cat.id} className="flex flex-col gap-2 group cursor-pointer" onClick={() => { setMainView('analytics'); setSubpage('categories'); }}>
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-xl flex items-center justify-center bg-[var(--accent-muted)] text-[var(--accent)] transition-colors group-hover:bg-[var(--accent)] group-hover:text-white border border-[var(--accent)]/10">
                      <cat.icon size={16} />
                    </div>
                    <span className="text-sm font-medium text-[var(--text-main)] group-hover:text-[var(--accent)] transition-colors">{cat.name}</span>
                  </div>
                  <span className="text-sm font-bold text-[var(--text-main)]">{showBalance ? formatCurrency(cat.amount) : '••••'}</span>
                </div>
                <div className="pl-12">
                  <ProgressBar percentage={cat.percentage} colorHex="var(--accent)" />
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="interactive-card bg-[var(--bg-surface)] border border-[var(--border-color)] rounded-[2rem] p-6 sm:p-8 flex flex-col gap-4 shadow-sm lg:col-span-2">
          <div className="flex justify-between items-center mb-1">
            <h3 className="text-base font-semibold text-[var(--text-main)]">Últimas Movimentações</h3>
            <button onClick={() => { setMainView('carteiras'); }} className="text-xs font-medium text-[var(--text-muted)] hover:text-[var(--text-main)] transition-colors">
          ...
          <div className="flex flex-col gap-1.5">
            {recentTransactions.map((tx: Transaction) => (
              <div key={tx.id} className="flex items-center justify-between p-3 sm:px-4 rounded-2xl hover:bg-[var(--bg-elevated)] border border-transparent hover:border-[var(--border-light)] transition-all cursor-pointer">
                <div className="flex items-center gap-3.5">
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center border ${tx.type === 'income' ? 'bg-[var(--income-muted)] text-[var(--income)] border-[var(--income)]/20' : 'bg-[var(--bg-base)] text-[var(--text-main)] border-[var(--border-light)]'}`}>
                    <tx.icon size={18} />
                  </div>
                  <div className="flex flex-col gap-0.5">
                    <span className="text-sm font-medium text-[var(--text-main)] leading-none">{tx.description}</span>
                    <span className="text-[11px] font-normal text-[var(--text-muted)]">{tx.category} • {tx.date}</span>
                  </div>
                </div>
                <span className={`text-sm font-bold ${tx.type === 'income' ? 'text-[var(--income)]' : 'text-[var(--text-main)]'}`}>
                  {showBalance ? `${tx.type === 'income' ? '+' : '-'}${formatCurrency(tx.amount)}` : '••••'}
                </span>
              </div>
            ))}
          </div>
        ...
        <div className="interactive-card bg-gradient-to-b from-[var(--accent-muted)] to-transparent border border-[var(--accent)]/20 rounded-[2rem] p-6 sm:p-8 flex flex-col gap-5 shadow-sm">
          <h3 className="text-base font-semibold text-[var(--text-main)] flex items-center gap-2">
            <Bell size={18} className="text-[var(--accent)]" /> Fique de Olho
          </h3>
          
          <div className="flex flex-col gap-3">
            <div className="p-4 bg-[var(--bg-surface)] rounded-2xl border border-[var(--accent)]/20 flex flex-col gap-1.5 shadow-sm hover:border-[var(--accent)] transition-colors cursor-pointer">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-[var(--accent)] uppercase tracking-wider bg-[var(--accent-muted)] px-2 py-0.5 rounded">Vence Hoje</span>
              </div>
              <span className="text-sm font-medium text-[var(--text-main)]">Fatura Cartão Black</span>
              <span className="text-xs font-normal text-[var(--text-muted)]">Valor: <span className="font-bold text-[var(--text-main)]">{showBalance ? formatCurrency(3240.50) : '••••'}</span></span>
            </div>

            <div className="p-4 bg-[var(--bg-surface)] rounded-2xl border border-[var(--border-light)] flex flex-col gap-1.5 shadow-sm hover:border-[var(--border-hover)] transition-colors cursor-pointer">
              <div className="flex items-center justify-between">
                <span className="text-[10px] font-bold text-[var(--warning)] uppercase tracking-wider bg-[var(--warning-muted)] px-2 py-0.5 rounded">Atenção</span>
              </div>
              <span className="text-sm font-medium text-[var(--text-main)]">Orçamento de Lazer</span>
              <span className="text-xs font-normal text-[var(--text-muted)] leading-relaxed">Você atingiu 90% do limite definido para este mês.</span>
            </div>
          </div>
        </div>
...
// COMPONENTES EXISTENTES: ... (Mantidos inalterados)
...
export default function FinanceProAnalytics() {
  ...
  const [currentMainView, setCurrentMainView] = useState<'dashboard' | 'analytics' | 'carteiras' | 'mais'>('dashboard');
    ...
    <div className={`min-h-screen ${isDark ? 'theme-dark' : 'theme-light'} bg-[var(--bg-base)] text-[var(--text-main)] relative selection:bg-[var(--accent-muted)] selection:text-[var(--accent)]`}>
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-4 flex flex-col gap-1 scrollbar-hide">
          ...
          <button 
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors w-full text-left font-medium ${currentMainView === 'dashboard' ? 'bg-[var(--accent-muted)] text-[var(--accent)]' : 'text-[var(--text-muted)] hover:bg-[var(--bg-elevated)] hover:text-[var(--text-main)]'}`}
            onClick={() => {
          ...
          <button 
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-colors w-full text-left font-medium ${currentMainView === 'analytics' ? 'bg-[var(--accent-muted)] text-[var(--accent)]' : 'text-[var(--text-muted)] hover:bg-[var(--bg-elevated)] hover:text-[var(--text-main)]'}`}
            onClick={() => {
              setCurrentMainView('analytics');
      ...
      <main className="w-full">
        {currentMainView === 'dashboard' && (
          <div className="max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pb-24 lg:pb-10">
            <DashboardPage data={data} setMainView={setCurrentMainView} setSubpage={setCurrentSubpage} />
          </div>
        )}
        ...
        {currentMainView === 'analytics' && (
          <div className="w-full animate-fade-in-up">
            ...
            <ContextualAnalyticsHeader 
              ...
              setSubpageToReports={() => setCurrentSubpage('reports')}
            />
            ...
            <div className="relative z-10 max-w-[1400px] mx-auto px-4 sm:px-6 lg:px-8 py-6 pb-24 lg:pb-10">
              ...
              {currentSubpage === 'reports' && <ReportsSubpage />}
              {currentSubpage === 'recurrences' && <RecurrencesSubpage recurrences={data.recurrences} />}
            </div>
          </div>
        )}

        {(currentMainView === 'carteiras' || (currentMainView === 'mais' && window.innerWidth >= 1024)) && (
           <div className="p-6 text-center text-[var(--text-muted)] pt-20 animate-fade-in-up max-w-[1400px] mx-auto">Em construção...</div>
        )}
